...
# 昼の運用
class EventWeekNoon(EventClass):
    ...
    def run_train(hour,min, stationid):
        ...
        if tc.timesig(6,11, hour, min) == True:
            del self.un5
            self.un5 = tr.UnyouClass(dp.goku.pull_car(),dp.goku.pull_car(), 10, 5)
        ...
        if tc.timesig(9, 7, hour, min) == True:
            dp.goku.push_cars(self.un6.change_all_cars(20, dp.goku.pull_car(), stationid))

        if tc.timesig(9, 24, hour, min) == True:
            self.un6.add_cars(26, dp.E04B.pull_car(), stationid)

        if tc.timesig(17, 50, hour, min) == True:
            dp.goku.push_car(self.un3.parge_cars(11, stationid))

        if tc.timesig(18,  14, hour, min) == True:
            dp.goku.push_car(self.un5.parge_cars(11, stationid))

        if tc.timesig(19,  0, hour, min) == True:
            dp.E04B.push_car(self.un6.parge_cars(26, stationid))

        if tc.timesig(18, 38, hour, min) == True:
            dp.goku.push_car(self.un1.parge_cars(11, stationid))
